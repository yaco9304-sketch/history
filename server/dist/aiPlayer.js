...
// AI 플레이어 생성
export function createAIPlayer(nation, difficulty = 'normal') {
...
// AI가 선택지를 평가하는 함수
export function evaluateChoice(choice, nation, currentStats, event, difficulty = 'normal') {
    let score = 0;
    // 1. 스탯 변화 평가
    const statWeights = {
    ...
    const minStat = Math.min(currentStats.military, currentStats.economy, currentStats.diplomacy, currentStats.culture);
    if (minStat < 50) {
        // 가장 낮은 스탯을 올리는 선택지에 보너스
        const effects = choice.effects;
        if (currentStats.military === minStat && (effects.military || 0) > 0)
            score += 30;
        if (currentStats.economy === minStat && (effects.economy || 0) > 0)
            score += 30;
        if (currentStats.diplomacy === minStat && (effects.diplomacy || 0) > 0)
            score += 30;
        if (currentStats.culture === minStat && (effects.culture || 0) > 0)
            score += 30;
    }
    // 5. 난이도별 랜덤성 추가
...
export function selectBestChoice(event, nation, currentStats, difficulty = 'normal') {
...
// AI가 자동으로 행동할지 결정
export function shouldAIAct(room, playerId) {
