...
// AI 조언자 프롬프트 생성
function createAdvisorPrompt(
  event: HistoricalEvent,
  nation: Nation,
  currentStats: NationStats,
  choices: Array<{ id: string; text: string; effects: Partial<NationStats>; risk: string }>
): string {
  const nationNames: Record<Nation, string> = {
    goguryeo: '고구려',
    baekje: '백제',
    silla: '신라',
  };
...
${choices.map((choice, i) => 
  `${i + 1}. ${choice.text} (효과: ${JSON.stringify(choice.effects)}, 위험도: ${choice.risk})`
).join('\n')}
...
export async function getAIAdvice(
  ...
  nation: Nation,
  currentStats: NationStats
): Promise<{ advice: string; isQuotaError?: boolean } | null> {
  const ai = getGenAI();
  if (!ai) {
    return null;
  }
  ...
  try {
    ...
    const prompt = createAdvisorPrompt(
      event,
      nation,
      currentStats,
      event.choices.map(c => ({
