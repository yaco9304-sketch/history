...
import { createAIPlayer, selectBestChoice, AIDifficulty } from './aiPlayer.js';
import {
...
// 국가 이름 매핑
const NATIONS: Record<Nation, { name: string }> = {
  goguryeo: { name: '고구려' },
  baekje: { name: '백제' },
  silla: { name: '신라' },
};
...
const debouncedRoomUpdate = (roomId: string, room: any) => {
  // 기존 타이머가 있으면 취소
  ...
  const timer = setTimeout(() => {
    console.log(`[Server] Sending roomUpdated to room ${roomId}:`, {
      ...
      aiPlayersCount: room ? Object.values(room.players || {}).filter((p: any) => p.isAI).length : 0
    });
 ...
 */
function isAIBattleMode(roomId?: string): boolean {
...
function selectEventForTurn(turn: number, nationsPresentInGame: Nation[], roomId?: string): HistoricalEvent {
  // 전체 이벤트 개수 로그
  ...
  // AI 대전 모드: 플레이어 국가 우선
  let playerNation: Nation | null = null;
  ...
  if (isAIBattle && playerNation) {
    ...
    const playerEvents = SAMPLE_EVENTS.filter(e =>
      e.targetNation === playerNation
    );
  ...
  const availableEvents = SAMPLE_EVENTS.filter(e =>
    e.targetNation === 'all' || nationsPresentInGame.includes(e.targetNation as Nation)
  );
...
// ============================================
const checkVoteCompletionAndAdvance = (roomId: string, nation: Nation): void => {
  const room = gameManager.getRoom(roomId);
        ...
        if (allNationsVoted) {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              const nations = ['goguryeo', 'baekje', 'silla'] as Nation[];
              // AI 대전 모드: 모든 국가가 활성화되어 있음 (AI 플레이어 포함)
              const activeNations = nations.filter(nation =>
                currentRoom.teams[nation].players.some(pid => {
...
// AI 플레이어 자동 투표 헬퍼 함수
// ============================================
const autoAIVote = (roomId: string, event: HistoricalEvent, delayMs: number = 2000): void => {
  const room = gameManager.getRoom(roomId);
  if (!room || !room.currentEvent) return;
      ...
      if (player && player.isAI) {
        ...
        if (!room.currentVotes[nation].votes[playerId]) {
          ...
          setTimeout(() => {
            ...
            // AI가 최선의 선택지 선택
            const bestChoice = selectBestChoice(
              event,
              nation,
              currentRoom.teams[nation].stats,
...
app.get('/api/teacher/dashboard/:roomCode', (req, res) => {
  try {
    ...
    // 국가별 진행상황
    const nationProgress = (['goguryeo', 'baekje', 'silla'] as Nation[]).map(nation => {
...
app.post('/api/ai/add', express.json(), (req, res) => {
  try {
    ...
    // 팀 인원 확인
    const teamPlayers = room.teams[nation as Nation].players;
    if (teamPlayers.length >= room.settings.maxPlayersPerTeam) {
      return res.status(400).json({ error: '해당 팀이 이미 가득 찼습니다.' });
    }
    
    // AI 플레이어 생성
    const aiPlayer = createAIPlayer(nation as Nation, (difficulty || 'normal') as AIDifficulty);
    ...
    // 팀 선택
    const teamSelected = gameManager.selectTeam(aiPlayer.id, nation as Nation);
    if (!teamSelected) {
    ...
    const cleanRoom = {
      ...
      players: Object.fromEntries(
        Object.entries(updatedRoom.players).filter(([pid, p]) =>
          p.isOnline &&
    ...
    console.log(`[API] AI player added and roomUpdated scheduled: ${aiPlayer.name} to ${roomCode}`, {
      ...
      aiPlayers: Object.values(cleanRoom.players).filter((p: any) => p.isAI).length
    });
...
app.post('/api/ai/remove', express.json(), (req, res) => {
  try {
    ...
    const cleanRoom = {
      ...result.room,
      players: Object.fromEntries(
        Object.entries(result.room.players).filter(([pid, p]) => p.isOnline && !p.isAI)
      ),
    };
...
const pendingBattles = new Map<string, { attackerNation: Nation; defenderNation: Nation; roomId: string }>();
...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', ({ code, playerName, playerId: providedPlayerId }) => {
    try {
      ...
      const cleanRoom = {
        ...room,
        players: Object.fromEntries(
          Object.entries(room.players).filter(([pid, p]) => 
            p.isOnline && 
            !pid.startsWith('test-host-')
  ...
  // 팀 선택
  socket.on('selectTeam', (nation: Nation) => {
    const playerId = socket.data.playerId;
        ...
        if (started) {
          ...
          if (updatedRoom) {
            ...
            setTimeout(() => {
              ...
              const nations = ['goguryeo', 'baekje', 'silla'] as Nation[];
              const activateNations = nations.filter(nation =>
                currentRoom.teams[nation].players.some(pid => {
                  const player = currentRoom.players[pid];
                  return player && (player.isOnline || player.isAI);
                })
  ...
  // 투표 제출
  socket.on('submitVote', (choiceId: string) => {
    try {
      const playerId = socket.data.playerId;
      const roomId = socket.data.roomId;
      if (!playerId || !roomId) {
  ...
  // 동맹 제안
  socket.on('proposeAlliance', (targetNation: Nation) => {
    const playerId = socket.data.playerId;
    const roomId = socket.data.roomId;
    if (!playerId || !roomId) return;
    ...
    if (success) {
      // 동맹 제안 메시지 생성
      const message = gameManager.createChatMessage(
        roomId,
        playerId,
        `${NATIONS[targetNation].name}에 동맹을 제안했습니다.`,
        'diplomacy',
        targetNation
      );
      if (message) {
  ...
  // 동맹 수락
  socket.on('acceptAlliance', (proposerNation: Nation) => {
    const playerId = socket.data.playerId;
    const roomId = socket.data.roomId;
    if (!playerId || !roomId) return;
    ...
    if (success) {
      ...
      const message = gameManager.createChatMessage(
        roomId,
        playerId,
        `${NATIONS[proposerNation].name}와 동맹을 맺었습니다!`,
        'system'
      );
      if (message) {
  ...
  // 동맹 파기
  socket.on('breakAlliance', (targetNation: Nation) => {
    const playerId = socket.data.playerId;
    const roomId = socket.data.roomId;
    if (!playerId || !roomId) return;
    ...
    if (success) {
      const message = gameManager.createChatMessage(
        roomId,
        playerId,
        `${NATIONS[targetNation].name}와의 동맹을 파기했습니다.`,
        'system'
      );
      if (message) {
        io.to(roomId).emit('chatMessage', message);
  ...
  // 적대 선포
  socket.on('declareWar', (targetNation: Nation) => {
    const playerId = socket.data.playerId;
    const roomId = socket.data.roomId;
    if (!playerId || !roomId) return;
    ...
    if (success) {
      const message = gameManager.createChatMessage(
        roomId,
        playerId,
        `${NATIONS[targetNation].name}에 전쟁을 선포했습니다!`,
        'system'
      );
      if (message) {
        io.to(roomId).emit('chatMessage', message);
  ...
  // 적대 해제
  socket.on('endWar', (targetNation: Nation) => {
    const playerId = socket.data.playerId;
    const roomId = socket.data.roomId;
    if (!playerId || !roomId) return;
    ...
    if (success) {
      const message = gameManager.createChatMessage(
        roomId,
        playerId,
        `${NATIONS[targetNation].name}와의 전쟁을 종료했습니다.`,
        'system'
      );
      if (message) {
        io.to(roomId).emit('chatMessage', message);
  ...
  // 전투 제안
  socket.on('initiateBattle', (defenderNation: Nation) => {
    const playerId = socket.data.playerId;
    const roomId = socket.data.roomId;
    if (!playerId || !roomId) return;
    ...
    if (result) {
      ...
      const message = gameManager.createChatMessage(
        roomId,
        playerId,
        `${NATIONS[defenderNation].name}에 전투를 제안했습니다!`,
        'system'
      );
      if (message) {
        io.to(roomId).emit('chatMessage', message);
  ...
  // 전투 수락
  socket.on('acceptBattle', (battleId: string) => {
    const playerId = socket.data.playerId;
    const roomId = socket.data.roomId;
    if (!playerId || !roomId) return;
    ...
    // 전투 결과 계산
    const battleResult = gameManager.calculateBattleResult(
      roomId,
      battle.attackerNation,
      battle.defenderNation
    );

    if (battleResult) {
      // 전투 결과 적용
      gameManager.applyBattleResult(
        roomId,
        battle.attackerNation,
        battle.defenderNation,
        battleResult
      ...
      const message = gameManager.createChatMessage(
        roomId,
        playerId,
        `${NATIONS[battleResult.winner].name}가 전투에서 승리했습니다!`,
        'system'
      );
      if (message) {
        io.to(roomId).emit('chatMessage', message);
  ...
  // 전투 거절
  socket.on('rejectBattle', (battleId: string) => {
    const playerId = socket.data.playerId;
    const roomId = socket.data.roomId;
    if (!playerId || !roomId) return;

    const battle = pendingBattles.get(battleId);
    if (!battle || battle.roomId !== roomId) return;
    ...
    // 방어자만 거절 가능
    if (player.team !== battle.defenderNation) return;
    ...
    const message = gameManager.createChatMessage(
      roomId,
      playerId,
      `${NATIONS[battle.attackerNation].name}의 전투 제안을 거절했습니다.`,
      'system'
    );
    if (message) {
      io.to(roomId).emit('chatMessage', message);
...
httpServer.listen(PORT, () => {
  ...
  try {
    ...
    if (!existingRoom) {
      const mainRoom = gameManager.createRoom(
        'system',
        '시스템',
        '기본 게임',
        undefined,
