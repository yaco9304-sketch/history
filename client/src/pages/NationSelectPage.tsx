...
export const NationSelectPage = () => {
  ...
  // 방 자동 입장은 하지 않음 (사용자가 국가 선택 후 입장)
  ...
  const handleJoinTeam = async () => {
    if (selectedNation && playerName.trim() && !isJoining) {
      ...
      try {
        ...
        console.error('[NationSelect] Join error:', err);
        setIsJoining(false);
                ...
                <div className="relative p-6 md:p-8">
                  ...
                  >
                    <img 
                      ...
                      style={{ display: 'block' }}
                      onError={(e) => {
                        console.error(`[NationSelect] Image failed to load: ${nationImages[nationId]}`, e);
                        ...
                        if (parent && !parent.querySelector('.fallback-icon')) {
                          ...
                          fallback.className = 'fallback-icon w-32 h-32 md:w-40 md:h-40 flex items-center justify-center text-6xl font-bold text-white/70 border-2 border-white/30 rounded-lg bg-black/30';
                          // 국가별 대체 아이콘
                          const icons: Record<Nation, string> = {
                            goguryeo: '🦅',
                            baekje: '🏛️',
                            silla: '👑'
                          };
                          fallback.textContent = icons[nationId] || '🏛️';
                          parent.appendChild(fallback);
                        }
                      }}
                      onLoad={() => {
                        console.log(`[NationSelect] Image loaded successfully: ${nationImages[nationId]}`);
                      }}
                    />
                  </motion.div>
